---
description: Invoke the crazy-professor divergence generator on a topic. Default: 10 provocations + 3 extracted concepts. With --chat: all four voices and 20 distilled ideas. --dial 0-100 tunes the wild/tame cost mix. --lab: open the static review browser. --harvest: triage pending runs and land kept experiments.
argument-hint: [topic] [--chat] [--dial 0-100] [--lab] [--harvest]
---
...
- If `$ARGUMENTS` contains a topic and no `--chat`, dispatch to single-run mode: one active archetype, one provocation word, one PO operator, exactly 10 provocations, 3 extracted concepts, and one next experiment.
